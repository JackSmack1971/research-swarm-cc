...
This skill may inspect, profile, research, route, record decisions, compile task plans, and produce an authorization record. It must not implement production code, modify product behavior, execute production changes, or approve its own work. It must not commit or push, merge, or deploy. Authorization is bounded future-execution context, not an executor or permission boundary.
