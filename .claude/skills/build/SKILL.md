...
3. Follow exactly one route: no inquiry for resolved/reversible details; repository inspection for codebase facts; Research Swarm and a scoped evidence packet only for material external facts; a disposable prototype for experiential, UX, state, or architecture questions; one highest-value human question for normative, preference, policy, hard-to-reverse, or consequential product choices; and an agent choice only for reversible, low-risk implementation details.
