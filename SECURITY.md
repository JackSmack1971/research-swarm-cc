...
Distribution is fail-closed: target paths are containment-checked and symlinks are refused, source assets must be regular files, ownership is recorded with version and SHA-256 hashes, conflicts and changed owned files stop lifecycle operations, updates create recoverable backups, and uninstall removes only unchanged owned bootstrap files. Target settings, hooks, archives, learning state, and other project data are never owned.
