...
Milestone 60 adds canonical, conditional production risk-profile gates to authorization. Authentication/authorization/security, sensitive data/privacy/integrity, schema/data migration, external API integration, UI/accessibility, infrastructure/deployment configuration, and dependency/supply-chain profiles activate only from recorded non-`none` risk dimensions. Each gate records activation evidence, planning constraints, required independent verification and evidence kinds, human merge/deploy boundaries, and a stop-and-recovery expectation. Missing, altered, or unproven gates fail deterministic authorization or verification; low-risk tasks retain no unrelated profile gates.

Milestone 61 adds a greenfield plan contract outside the research archive. It captures a concise charter and user constraints, retains stack decisions with evidence and alternatives, records the Decision Router's conditional research choice, and makes subtraction-ladder results explicit. A valid accepted Change Contract/program contract and a safe relative file set are required before native scaffolding. The generated target is profiled again and then uses the existing task graph, risk authorization, isolated executor, fresh verifier, and delivery handoff; no autonomous deployment or second architecture is introduced.

Milestone 62 adds a separate deterministic engineering-learning registry. It accepts only structured signals bound to a digest-validated Delivery-plane handoff and records provenance as `synthetic_fixture`, `plain_claude_baseline`, or `live_delivery`. The first two kinds can exercise validation and lifecycle rejection but can never promote or activate learning. `live_delivery` additionally requires a complete execution event, independently proven criteria, explicit manifest provenance, and a matching Claude Code session attestation. Provisional lessons move through review, promotion, rejection, and rollback only under bounded applicability and protected-surface checks. The registry is dormant until a separate final Claude Code project-acceptance attestation enables it; it never changes Intent, authorization, risk controls, research learning, merge, or deployment.
...
Milestone 69 adds retrieval-outcome signals to the separate engineering-learning plane. A signal is bound to a completed Knowledge Need and records only route/process facts: need class and risk/freshness, selected T0–T4 route and mechanism, Graphify use/reason, bounded agent/source counts, nullable context/token/latency telemetry, accepted-decision and independently-proven-criterion outcomes, repair/rework, and human usefulness/dead-end feedback. It may optionally link a validated downstream Delivery manifest. The signal has explicit hashes and provenance, rejects conflicting or insufficient evidence, and contains no research claims, reports, sources, or memory directives. Synthetic and baseline signals remain non-promoting; any bounded routing preference remains dormant until the existing live-delivery and final-acceptance gates pass. Research learning remains responsible for behavior inside selected research tiers, and neither plane can weaken provenance, security, risk, independent verification, or human decisions.
