...
| Route simple work to cheaper models | implemented | Milestone 40 centrally routes retrieval/filtering to `haiku`, evidence reasoning to `sonnet`, and planning/synthesis to the session model. Do not add a global `CLAUDE_CODE_SUBAGENT_MODEL` override because it supersedes every stage route. Continue measuring quality baselines before changing this policy. |
