...
| Route simple work to cheaper models | revised after runtime evidence | A Haiku-routed `agent({schema})` stage exhausted all five structured-output retries in Milestone 46 acceptance. Milestone 40 therefore routes every substantive schema-producing/canonical-output stage to `sonnet`; planning and synthesis retain intentional session-model inheritance. Do not add a global `CLAUDE_CODE_SUBAGENT_MODEL` override because it supersedes every stage route. This is workflow-specific effective-cost evidence, not a universal claim about Haiku. |
