...
try {
  ...
  }, runVerifier: async ({ input, worktreePath }) => { const result = await launch('engineering-verifier', `Return JSON only with { event, proofs }. Verify this supplied context only; executor reasoning is unavailable.\n${JSON.stringify(input)}`, worktreePath, 'sonnet', 'medium', 'default'); if (result.exit_code) throw new Error('Independent verifier did not complete.'); return JSON.parse(result.stdout); }, runRepair: async ({ defects, worktreePath, capsule: task }) => { const result = await launch('engineering-executor', `Repair only these identified defects in this authorized capsule. Do not verify, commit, merge, push, deploy, or change planning state.\n${JSON.stringify({ task, defects })}`, worktreePath); if (result.exit_code) return null; return { change_identity: await changeIdentity(worktreePath) }; } });
