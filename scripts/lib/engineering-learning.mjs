...
const SIGNALS = new Set(['review_finding', 'repair_rework', 'failed_acceptance', 'runtime_regression', 'user_correction', 'human_friction', 'complexity_problem', 'post_delivery_outcome']);
...
export async function registerEngineeringEvidence({ root = engineeringLearningRoot(), evidence, lesson }) {
  ...
  const handoff = await loadDeliveryHandoff(evidence.manifest_path, evidence.target_path); const rawManifest = await readFile(evidence.manifest_path, 'utf8'); const manifest = handoff.manifest; if (evidence.signal_source.kind === 'delivery_manifest' && evidence.signal_source.record_id !== manifest.delivery_id) throw new Error('Signal source does not identify the supplied delivery manifest.');
  ...
  const record = { ...evidence, manifest_sha256: hash(rawManifest), delivery_id: manifest.delivery_id, execution_event_id: manifest.final_change.execution_event_id, verification_event_ids: [...manifest.verification.event_ids], criterion_proof_ids: manifest.verification.proof_statuses.map(({ proof_id }) => proof_id), live_eligible: liveEligible, accepted_for_learning: false };
