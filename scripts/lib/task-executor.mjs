...
export async function executeAuthorizedTask({ authorization, contract, graph, capsule, targetPath, signals, runExecutor, protectedPaths = ['engineering/contracts', 'engineering/authorizations', 'engineering/tasks', 'engineering/evidence'] }) {
  ...
  try {
    ...
    const change_identity = await git(worktreePath, ['diff', '--binary']).then((value) => value ? `diff:${Buffer.from(value).toString('base64url')}` : null);
    return { events: [preflight, event('complete', base_revision, capsule.task_id, worktree, commands, file_changes, 'unverified_implementation', change_identity)], worktreePath };
