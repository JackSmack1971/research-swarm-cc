...
export async function inspectRepositoryKnowledge(need, targetPath, { file, text, symbol, dependency, lsp, now } = {}) {
  ...
  return result(need, profile, { anchors: [{ path: '.', kind: 'file', excerpt: 'Repository structure was profiled without inferred relationships.' }], observed_facts: [`Repository profile is revision-stamped at ${profile.target.git_revision ?? 'no Git revision'}.`], unresolved_subquestions: ['The requested structural relationship was not established by direct inspection or a configured LSP.'], confidence: 'low', provenance_type: 'direct_observation', mechanism: 'read', external_evidence_necessary: false });
}
