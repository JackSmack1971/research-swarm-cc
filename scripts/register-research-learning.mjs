...
import { learningRoot, mergeLesson, readState, safeLesson, withLearningLock, writePolicy, writeState } from './lib/research-learning.mjs';
...
const output = await withLearningLock(root, async () => { const state = await readState(root); const byKey = new Map(state.provisional.map((lesson) => [JSON.stringify([lesson.type, lesson.target?.policy_surface, lesson.target?.role, lesson.recommended_behavior?.trim().toLowerCase(), [...lesson.applicability_conditions].map((value) => value.trim().toLowerCase()).sort(), [...lesson.exclusions].map((value) => value.trim().toLowerCase()).sort()]), lesson])); for (const lesson of lessons) { const lessonKey = JSON.stringify([lesson.type, lesson.target?.policy_surface, lesson.target?.role, lesson.recommended_behavior?.trim().toLowerCase(), [...lesson.applicability_conditions].map((value) => value.trim().toLowerCase()).sort(), [...lesson.exclusions].map((value) => value.trim().toLowerCase()).sort()]); const existing = byKey.get(lessonKey); if (existing) Object.assign(existing, mergeLesson(existing, lesson)); else { state.provisional.push(lesson); byKey.set(lessonKey, lesson); } } await writeState(root, state); const policy = await writePolicy(root, state); return { registered: true, lessons: lessons.length, policy_bundle_id: policy.policy_bundle_id }; });
