...
| `npm run evidence:capsule -- <archive-directory> <knowledge-need.json> <claim-id[,claim-id...]> <selection-rationale>` | Emit the unified T4 Engineering Evidence Capsule; T1–T3 adapters are available from `scripts/lib/engineering-evidence-capsule.mjs`. | `scripts/compile-engineering-evidence-capsule.mjs` |
...
`/build` is manually invoked and does not implement code. It records material uncertainties for brownfield changes or a concise greenfield charter, routes repository facts to inspection, material external facts to Research Swarm/Evidence Capsules, experiential or architecture uncertainty to a disposable prototype, and only normative, preference, policy, hard-to-reverse, or consequential decisions to one human question. Evidence never becomes a decision automatically. The legacy T4 evidence-packet command remains read-only-compatible.
